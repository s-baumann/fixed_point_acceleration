...
/// Reference values were generated by the Julia implementation.  To regenerate
/// after algorithm changes, run the Julia rebase command and copy the output:
...
fn cross_language_regression() {
    ...
    let yaml_content = std::fs::read_to_string(&yaml_path).unwrap_or_else(|e| {
        panic!("Could not read tests/regression_refs.yaml at {}: {e}", yaml_path.display())
    });
